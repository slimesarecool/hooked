...
local uv = require("uv")
local wrapStream = require('hooked/coro-channel').wrapStream
local wrapper = require('hooked/coro-wrapper')
...
local function connect(options)
  ...
  local dsocket
  if options.tls then
    if not secureSocket then secureSocket = require('hooked/secure-socket/init') end
